...
if (isset($_GET['id'])) {
    ...
    $stmt = $conn->prepare("SELECT name, username, password, poste, email, statut, habitat FROM user WHERE id = ?");
    $stmt->bind_param("i", $id);
    $stmt->execute();
    $stmt->bind_result($name, $username, $password, $poste, $email, $statut, $habitat);
    $stmt->fetch();
    $stmt->close();
} else {
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if ($name && $username && $password && $poste && $email) {
        $stmt = $conn->prepare("UPDATE employe SET name = ?, username = ?, password = ?, poste = ?, email = ?, statut = ?, habitat = ? WHERE id = ?");
        $stmt->bind_param("ssssssi", $name, $username, $position, $role, $habitat, $email, $habitat, $id);
        $stmt->execute();
        $stmt->close();
